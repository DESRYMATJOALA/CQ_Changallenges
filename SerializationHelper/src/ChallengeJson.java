...
public class ChallengeJson  {
    ...
    private void writeToTXTFile(ArrayList<String> user) {
        String path = "C:\\workplace\\SerializationHelper\\src\\data.txt";
        String contents = "Name: "+user.get(0)+", "+
        "Surname: "+user.get(1)+", "+
        "Email: "+user.get(2)+", "+
        "Date_Of_Birth: "+user.get(3);

        try {
            SerializationHelper.serialiseToFile(contents,path);
        }catch (Exception e)
        {
            e.fillInStackTrace();
        }
